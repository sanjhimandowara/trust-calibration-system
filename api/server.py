...
@app.get("/grader")
def grader(task: Optional[str] = Query(default="medium")):
    task = (task or "medium").lower()
    if task == "easy":
        return {"score": clamp_score(0.65 if TASK_METRICS["total"] == 0 else grade_easy(TASK_METRICS))}
    if task == "hard":
        return {"score": clamp_score(0.55 if TASK_METRICS["total"] == 0 else grade_hard(TASK_METRICS))}
    return {"score": clamp_score(0.75 if TASK_METRICS["total"] == 0 else grade_medium(TASK_METRICS))}


@app.get("/grader/easy")
def grader_easy_endpoint():
    score = 0.65 if TASK_METRICS["total"] == 0 else grade_easy(TASK_METRICS)
    return {"score": clamp_score(score)}


@app.get("/grader/medium")
def grader_medium_endpoint():
    score = 0.75 if TASK_METRICS["total"] == 0 else grade_medium(TASK_METRICS)
    return {"score": clamp_score(score)}


@app.get("/grader/hard")
def grader_hard_endpoint():
    score = 0.55 if TASK_METRICS["total"] == 0 else grade_hard(TASK_METRICS)
    return {"score": clamp_score(score)}
